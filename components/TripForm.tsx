...
const TripForm: React.FC<TripFormProps> = ({ onSave, activeTrip, vehicles, volunteers }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isAltro) {
      finalReason = formData.customReason || 'Altro';
    } else {
      const cat = SERVICE_CATEGORIES.find(c => c.icon === formData.icon);
      finalReason = cat ? cat.label : 'Servizio';
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
      <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-5">
          ...
          <>
            ...
            {isAltro && (
              <div className="animate-in slide-in-from-top-2">
                <label className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1 block">Specifica Motivo</label>
                <input
                  type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 block">KM Partenza</label>
                <input
                  type="number"
                  inputMode="numeric"
        ...
        <div className="grid grid-cols-1 gap-3 pt-2">
            ...
            {isEnding && (
              <div className="space-y-3">
                ...
                >
                    <span className="text-xs font-black uppercase text-red-600">Segnala Guasto</span>
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ${formData.maintenanceNeeded ? 'bg-red-500 text-white' : 'bg-gray-200 text-transparent'}`}>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                        </svg>
                    </div>
                </button>
                {formData.maintenanceNeeded && (
                    <textarea 
                        ...
                        placeholder="Descrivi il guasto o l'anomalia..."
        ...
        <div>
          <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 block">Note Finali</label>
          <textarea
      ...
      >
        {isEnding ? 'CHIUDI MISSIONE' : 'AVVIA SERVIZIO'}
      </button>
    </form>
  );
};
